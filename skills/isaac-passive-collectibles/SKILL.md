...
For declared REPENTOGON passives, use `isaac-repentogon-content` for native XML
stats, effect stats and custom cache, and `isaac-repentogon-callbacks` for true
inventory, item-wisp and innate ownership events. Matching-build native formulas
and event semantics take precedence over vanilla-only implementation advice.
Give each contribution one owner and preserve its formula layer, balance and
loss behavior; an inventory notification is not proof of a natural pickup.
