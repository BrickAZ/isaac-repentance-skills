...
Use `isaac-repentogon-dev` for new-project setup, requested migration or broad
REPENTOGON implementation, and `isaac-repentogon-compat` for build/capability and
Lua/XML loading evidence. A standing required choice applies to new mods without
reasking; preserve an existing explicit optional policy unless migration is
requested. Missing files are discovery facts, not invented existing structure.
Selected matching-build contracts take precedence over vanilla-only assumptions.
